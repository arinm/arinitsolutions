    ...
    <>
      ...
      <section className="py-16 md:py-24">
        <Container className="max-w-[720px]">
          ...
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 rounded-full bg-neutral-800 flex items-center justify-center text-sm font-medium text-accent-400">
      ...
      <section className="pb-16 md:pb-20">
        <Container className="max-w-[680px]">
          <article className="[&>h2]:text-2xl [&>h2]:font-bold [&>h2]:text-neutral-50 [&>h2]:mt-12 [&>h2]:mb-4 [&>h3]:text-xl [&>h3]:font-semibold [&>h3]:text-neutral-50 [&>h3]:mt-8 [&>h3]:mb-3 [&>p]:text-neutral-300 [&>p]:leading-[1.7] [&>p]:mb-4 [&>ul]:space-y-2 [&>ul]:mb-4 [&>ul>li]:text-neutral-300 [&>ul>li]:pl-1 [&>ol]:space-y-2 [&>ol]:mb-4 [&>ol>li]:text-neutral-300 [&>blockquote]:border-l-4 [&>blockquote]:border-accent-500 [&>blockquote]:pl-4 [&>blockquote]:italic [&>blockquote]:text-neutral-300 [&>blockquote]:my-6 [&>pre]:bg-neutral-800 [&>pre]:rounded-lg [&>pre]:p-4 [&>pre]:overflow-x-auto [&>pre]:my-6 [&>pre]:text-sm [&>code]:bg-neutral-800 [&>code]:text-accent-300 [&>code]:px-1.5 [&>code]:py-0.5 [&>code]:rounded [&>code]:text-sm [&_a]:text-accent-400 [&_a]:underline-offset-4 [&_a:hover]:underline">
      ...
      <section className="py-12 border-t border-neutral-700/50">
        <Container className="max-w-[680px]">
          <div className="flex items-center gap-4">
            <div className="h-14 w-14 rounded-full bg-neutral-800 border-2 border-neutral-700 flex items-center justify-center text-lg font-medium text-accent-400">
              {post.author.name.split(" ").map((n) => n[0]).join("")}
            </div>
            <div>
              <p className="text-base font-medium text-neutral-100">{post.author.name}</p>
      ...
      {related.length > 0 && (
        <section className="py-16 md:py-20 border-t border-neutral-700/50">
          <Container>
            <h2 className="text-2xl font-bold text-neutral-50 mb-8">Keep reading</h2>
              ...
              {related.map((r) => (
                <Link key={r.slug} href={`/insights/${r.slug}`} className="block">
                  <Card variant="interactive" className="h-full">
                    ...
                    <h3 className="text-lg font-medium text-neutral-50 mb-2">{r.title}</h3>
